...
if regIniciado="s" then
    ...
    oEditRegistro.AbreTabela("SELECT id,nome,fotoCapa,fotoTopoPagina,resumo,texto,dataProjeto FROM "&oEditRegistro.prefixoTabela&"novidades WHERE ativo='s' AND id="&id)
else
    ...
    oEditRegistro.AbreTabela("SELECT id,nome,fotoCapa,fotoTopoPagina,resumo,texto,dataProjeto FROM "&oEditRegistro.prefixoTabela&"novidades WHERE ativo='s' AND userIdSession="&idUser&" AND registroIncluido='n' ORDER BY id DESC")
